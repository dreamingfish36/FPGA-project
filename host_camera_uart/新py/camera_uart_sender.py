...
# ===== 修改点 S05-10MODE-A：sobel_05 显示模式统一映射表 =====
# 这里的数字必须和 PL 端 hdmi_bram_sobel_display.v 中的 MODE_xxx 编号保持一致。
# 控制帧最终发送的是：a5 5a 01 <mode_value>。
#
# 目前按你要求保留 10 个模式：
# 0 原图 / 1 灰度处理 / 2 gauss 高斯滤波 / 3 midfilter 中值滤波 / 4 sobel
# 5 红色边缘叠加 / 6 bin 二值化 / 7 单色输出-R / 8 单色输出-G / 9 单色输出-B
DISPLAY_MODES: dict[str, int] = {
    "original": 0,          # MODE_ORIGINAL         原图
    "gray": 1,              # MODE_GRAY             灰度处理
    "gauss": 2,             # MODE_GAUSS_FILTER     gauss 高斯滤波
    "mid_filter": 3,        # MODE_MID_FILTER       midfilter 中值滤波
    "sobel": 4,             # MODE_SOBEL            sobel 边缘检测
    "edge_overlay_red": 5,  # MODE_EDGE_OVERLAY_RED 红色边缘叠加
    "binary": 6,            # MODE_BINARY           bin 二值化
    "red": 7,               # MODE_RED_ONLY         单色输出-R
    "green": 8,             # MODE_GREEN_ONLY       单色输出-G
    "blue": 9,              # MODE_BLUE_ONLY        单色输出-B
}

DISPLAY_MODE_DESCRIPTIONS: dict[str, str] = {
    "original": "原图",
    "gray": "灰度处理",
    "gauss": "gauss 高斯滤波",
    "mid_filter": "midfilter 中值滤波",
    "sobel": "sobel 边缘检测",
    "edge_overlay_red": "红色边缘叠加",
    "binary": "bin 二值化",
    "red": "单色输出-R",
    "green": "单色输出-G",
    "blue": "单色输出-B",
...
# 命令行仍然使用英文 key，例如 --mode gauss。
DISPLAY_MODE_CHOICES = tuple(DISPLAY_MODES.keys())

# GUI 下拉框显示“英文 key - 中文解释”，方便别人看懂。
DISPLAY_MODE_LABELS = tuple(
    f"{key} - {DISPLAY_MODE_DESCRIPTIONS[key]}" for key in DISPLAY_MODE_CHOICES
)
DISPLAY_MODE_LABEL_TO_KEY = {
    label: key for key, label in zip(DISPLAY_MODE_CHOICES, DISPLAY_MODE_LABELS)
}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        # ===== 修改点 S05-10MODE-B：命令行可选模式同步到 10 个 =====
        # 这里直接使用 DISPLAY_MODE_CHOICES，因此后续改 DISPLAY_MODES 后，命令行 choices 会自动同步。
        choices=DISPLAY_MODE_CHOICES,
        help=(
            "sobel_05 display mode control command. "
            "Available: " + ", ".join(DISPLAY_MODE_CHOICES)
        ),
    )
...
def send_requested_controls(
    ...
    # ===== 修改点 S05-10MODE-C：发送 mode 控制命令时使用 10 模式映射表 =====
    # GUI 传进来的是 “英文 key - 中文解释” 时，先转换回英文 key。
    # send_control_command 会发送：A5 5A 01 mode_value
    if mode is not None:
        mode_key = DISPLAY_MODE_LABEL_TO_KEY.get(mode, mode)
        if mode_key not in DISPLAY_MODES:
            valid_modes = ", ".join(DISPLAY_MODE_CHOICES)
            raise ValueError(f"unsupported display mode: {mode}. Valid modes: {valid_modes}")
        send_control_command(ser, CONTROL_MODE, DISPLAY_MODES[mode_key])
